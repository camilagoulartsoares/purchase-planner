  ...
  return (
    <AppShell>
      <nav className="mb-6 text-sm text-muted" aria-label="Navegação estrutural do produto">
        <Link to="/marcas" className="text-rose hover:underline" title="Ver todas as marcas">
          Marcas
        </Link>
      ...
      <div className="grid gap-8 lg:grid-cols-2">
        ...
        <div>
          ...
          <div className="mt-8 flex flex-wrap gap-3">
            ...
            {product.status !== "Já comprei" ? <button type="button" className="btn-ghost" title="Abrir análise de compra" aria-label={`Analisar se vale a pena comprar ${product.name}`} onClick={() => setSearchParams({ analysis: "1" })}>Vale a pena comprar?</button> : null}
            <button type="button" className="btn-ghost" onClick={() => navigate(-1)} title="Voltar para a página anterior" aria-label="Voltar para a página anterior">
              Voltar
            </button>
          </div>
        </div>
