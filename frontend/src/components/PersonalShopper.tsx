...
export function PersonalShopper() {
  ...
  const send=async()=>{
    ...
    }catch(e){setMessage(text);setError(e instanceof Error?e.message:"Não foi possível consultar as lojas.");}
  ...
  return <><button type="button" className="shopper-popup-trigger" onClick={()=>setOpen(true)}><Bot size={21}/><span>Personal Shopper IA</span></button>{open&&<div className="shopper-popup-backdrop" onMouseDown={()=>setOpen(false)}><div className="shopper-popup" onMouseDown={e=>e.stopPropagation()}><button className="shopper-popup-close" onClick={()=>setOpen(false)}><X size={20}/></button><section className="shopper-shell card-soft p-4 sm:p-5"><div className="shopper-heading"><div><p className="planner-kicker"><Bot size={15}/> Personal Shopper IA</p><h2 className="font-display mt-2 text-3xl font-semibold text-brown-deep">Encontre para mim</h2></div></div>{history.length?<div className="shopper-history">{history.slice(0,6).map(x=><button type="button" className={conversationId===x.id?"is-active":""} key={x.id} disabled={loading} onClick={()=>void openConversation(x.id)}>{x.title||"Busca sem título"}</button>)}</div>:null}<div className="shopper-chat">{messages.map((x,i)=><p key={i} className={`shopper-message ${x.role}`}>{x.content}</p>)}{loading&&<p className="shopper-message assistant"><LoaderCircle size={16} className="animate-spin"/> Carregando…</p>}</div><form className="shopper-form" onSubmit={e=>{e.preventDefault();void send();}}><input value={message} disabled={loading} onChange={e=>setMessage(e.target.value)} placeholder="Ex.: Quero uma bolsa elegante até R$ 200"/><button className="btn-primary" disabled={loading||!message.trim()}><Send size={16}/> Buscar</button></form>{loading&&<div className="shopper-loading" role="status" aria-live="polite"><LoaderCircle size={24} className="animate-spin"/><span>Buscando as melhores ofertas...</span><div className="shopper-loading-cards" aria-hidden="true"><i/><i/><i/></div></div>}{error&&<p className="finding-error mt-3">{error}</p>}{all.length?<><div className="shopper-results-head"><h3>Produtos e variações</h3><span>{shown.length} variações · {offers.length} ofertas</span></div><div className="shopper-controls"><label className="shopper-store-select">Loja<select value={store} onChange={e=>setStore(e.target.value)}><option value="">Todas as lojas</option>{merchants.map(x=><option key={x} value={x}>{x}</option>)}</select></label><label className="shopper-store-select">Ordenar por<select value={sort} onChange={e=>setSort(e.target.value as ShopperSort)}><option value="relevance">Relevância</option><option value="lowest_price">Menor preço</option><option value="highest_price">Maior preço</option><option value="best_rated">Mais bem avaliados</option><option value="most_reviews">Mais avaliações</option></select></label></div><div className="shopper-grid" ref={gridRef}>{shown.map(v=><article className="shopper-card" key={v.id}><div className="shopper-image">{v.imageUrl?<img src={v.imageUrl} alt={v.title}/>:<span>Imagem indisponível</span>}</div><div className="shopper-card-copy"><h4>{v.title}</h4></div><div className="shopper-offers">{v.offers.map(o=><div className="shopper-offer" key={o.id}><div><b>{o.store}</b><strong>{o.price==null?"Preço não informado":formatBRL(o.price)}</strong><OfferReview offer={o}/></div><div className="shopper-actions"><a className="btn-ghost" href={o.productUrl} target="_blank" rel="noreferrer"><ExternalLink size={14}/> Loja</a><button className="btn-ghost" onClick={()=>void act(o,"save")}><Heart size={14}/> Salvar</button><button className="btn-primary" onClick={()=>void act(o,"add-to-planner")}><Plus size={14}/> Planner</button></div></div>)}</div></article>)}</div></>:null}</section></div></div>}</>;
}
