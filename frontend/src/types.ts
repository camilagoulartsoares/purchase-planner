...
export type ShopperReply = { conversationId: string; query: ShopperQuery; answer: string; results: ShopperResult[]; variations: ShopperVariation[]; freshness?: { oldestCheckedAt: string | null; newestCheckedAt: string | null; agedCount: number; freshCount: number }; metrics?: { queries: string[]; searchCalls: number; detailCalls: number; rawResults: number; uniqueResults: number; variationCount: number; offerCount: number } | null; provider: string; suggestions: string[] };
