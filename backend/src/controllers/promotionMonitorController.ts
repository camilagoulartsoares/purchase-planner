...
export const promotionMonitorController = {
  ...
  async create(req: Request, res: Response, next: NextFunction) {
    try { const input = promotionWatchItemSchema.parse(req.body); const value = await prisma.promotionWatchItem.create({ data: { userId: req.user!.id, ...input } }); return ok(res, serializeItem(value), 201); } catch (error) { return next(error); }
  },
