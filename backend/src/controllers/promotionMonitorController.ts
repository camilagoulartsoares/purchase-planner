...
async function watchedItemWithOffers(userId: string, id: string, take = 1) {
  const item = await prisma.promotionWatchItem.findFirst({ where: { id, userId }, include: { offers: { orderBy: [{ totalPrice: "asc" }, { checkedAt: "desc" }], take } } });
...
export const promotionMonitorController = {
  ...
  async create(req: Request, res: Response, next: NextFunction) {
    try { const input = promotionWatchItemSchema.parse(req.body); const value = await prisma.promotionWatchItem.create({ data: { userId: req.user!.id, ...input } }); await promotionMonitorService.checkNow(req.user!.id, value.id); return ok(res, await watchedItemWithOffers(req.user!.id, value.id, 100), 201); } catch (error) { return next(error); }
  },
  ...
  async checkNow(req: Request, res: Response, next: NextFunction) {
    try { const id = paramId(req); await promotionMonitorService.checkNow(req.user!.id, id); return ok(res, await watchedItemWithOffers(req.user!.id, id, 100)); } catch (error) { return next(error); }
  },
