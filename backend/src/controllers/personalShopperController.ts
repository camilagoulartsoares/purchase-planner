...
export const personalShopperController = {
  ...
  async message(req: Request, res: Response, next: NextFunction) { const startedAt = Date.now(); res.once("finish", () => console.info("[shopper.http]", { status: res.statusCode, totalMs: Date.now() - startedAt, bytes: Number(res.getHeader("content-length")) || null })); try { const body = personalShopperMessageSchema.parse(req.body); return ok(res, await personalShopperService.message(req.user!.id, body.conversationId, body.message)); } catch (error) { return next(error); } },
  async refresh(req: Request, res: Response, next: NextFunction) { try { return ok(res, await personalShopperService.refresh(req.user!.id, String(req.params.id))); } catch (error) { return next(error); } },
  async discover(req: Request, res: Response, next: NextFunction) { try { return ok(res, await personalShopperService.discover(req.user!.id, String(req.params.id))); } catch (error) { return next(error); } },
