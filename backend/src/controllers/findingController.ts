...
export const findingController = {
  async preview(req: Request, res: Response, next: NextFunction) {
    try {
      const includeDebug = env.nodeEnv !== "production" || Boolean(env.linkImportDebugToken && req.header("x-link-import-debug") === env.linkImportDebugToken);
      return ok(res, await linkImportService.preview(String(req.body?.url || ""), includeDebug));
