...
export const purchaseAnalysisService = {
  async analyze(userId: string, productId: string, budget: number | null) {
    const rows = await prisma.product.findMany({ where: { userId, OR: [{ availability: null }, { availability: { not: "out_of_stock" } }] }, include: { brand: true, images: { orderBy: { position: "asc" } } } });
