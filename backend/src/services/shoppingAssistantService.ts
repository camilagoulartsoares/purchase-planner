...
export const shoppingAssistantService = {
  async ask(userId: string, message: string) {
    const rows = await prisma.product.findMany({ where: { userId, OR: [{ availability: null }, { availability: { not: "out_of_stock" } }] }, include: { brand: true, images: { orderBy: { position: "asc" } } }, orderBy: { createdAt: "desc" } });
